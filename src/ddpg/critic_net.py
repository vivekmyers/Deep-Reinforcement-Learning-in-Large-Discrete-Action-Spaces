...
class CriticNet:
    ...
    def __init__(self, num_states, num_actions, dim_embed):
        ...
        with self.g.as_default():
            ...
            self.E_c, self.embed_W, self.embed_B, self.W1_c, self.B1_c, self.W2_c, self.W2_action_c, self.B2_c, self.W3_c, self.B3_c,\
                self.critic_q_model, self.critic_state_in, self.critic_action_in = self.create_critic_net(
                    num_states, num_actions)

            # create target_q_model:
            self.t_E_c, self.t_embed_W, self.t_embed_B, self.t_W1_c, self.t_B1_c, self.t_W2_c, self.t_W2_action_c, self.t_B2_c, self.t_W3_c, self.t_B3_c,\
            ...
            self.act_grad_v = tf.gradients(self.critic_q_model, self.E_c)
            ...
            self.update_target_critic_op = [
                self.t_embed_W.assign(TAU * self.embed_W + (1 - TAU) * self.t_embed_W),
                self.t_embed_B.assign(TAU * self.embed_B + (1 - TAU) * self.t_embed_B),
            ...
            self.sess.run([
                self.t_embed_W.assign(self.embed_W),
                self.t_embed_B.assign(self.embed_B),
    ...
    def create_critic_net(self, num_states=4, num_actions=1):
        ...
        embed_W = tf.Variable(tf.random_uniform(
            [num_actions, DIM_EMBED], -1 / math.sqrt(num_actions), 1 / math.sqrt(num_actions)))
        embed_B = tf.Variable(tf.random_uniform(
            [DIM_EMBED], -1 / math.sqrt(num_actions), 1 / math.sqrt(num_actions)))
        ...
        E_c = tf.matmul(critic_action_in, embed_W) + embed_B
        H1_c = tf.nn.softplus(tf.matmul(critic_state_in, W1_c) + B1_c)
        H2_c = tf.nn.tanh(tf.matmul(H1_c, W2_c) + tf.matmul(E_c, W2_action_c) + B2_c)

        critic_q_model = tf.matmul(H2_c, W3_c) + B3_c

        return E_c, embed_W, embed_B, W1_c, B1_c, W2_c, W2_action_c, B2_c, W3_c, B3_c, critic_q_model, critic_state_in, critic_action_in

    def embed(self):
        def scale(arr):
            return self.sess.run(self.E_c, feed_dict={self.critic_action_in: arr})
        return scale
    ...
    def evaluate_target_critic(self, state_t_1, action_t_1):
        return self.sess.run(self.t_critic_q_model, feed_dict={self.t_critic_state_in: state_t_1, self.t_E_c: action_t_1})
    ...
    def compute_delQ_a(self, state_t, action_t):
        ...
        return self.sess.run(self.action_gradients, feed_dict={self.critic_state_in: state_t, self.E_c: action_t})
